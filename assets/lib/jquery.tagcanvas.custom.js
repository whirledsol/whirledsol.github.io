...
Tproto.Clicked = function(e) {
  ...
  if(t != '' && t != '_self') {
    if(self.frames[t]) {
	  ...
	  //CHANGED WR 3/14/2016
      //window.open(h, t);
	  if(this.tc.clickCallback != null){
		this.tc.clickCallback(a);
	  }
	}
    return;
  }
...
TCproto.GetTags = function() {
  ...
  for(k = 0; k <= this.repeatTags; ++k) {
    for(i = 0; i < el.length; ++i) {
	  //CUSTOM CHANGES WR 03/13/16: li
      etl = el[i].getElementsByTagName('li');
      for(j = 0; j < etl.length; ++j) {
		//CUSTOM CHANGES WR 03/13/16: doesn't include filteredOut items
		if(etl[j].className.indexOf("filteredOut") < 0){
			tl.push(etl[j]);
		}
      }
    }
...
TCproto.Load = function() {
  ...
  if(this.noTagsMessage && !taglist.length) {
    taglist = this.Message('Nothing here yet');
  }
